...
- FullCalendar.jsによるカレンダー表示（ステータス別色分け）
- **React + TypeScriptによるリアクティブな予約フォーム**
  - 患者選択時にアレルギー情報をリアルタイム表示
  - メニュー選択時に所要時間・料金・検査前注意事項を即座に表示
...
予約フォームで患者を選択した瞬間にアレルギー情報が赤いボックスで表示され、入力ミスを防ぐ設計にしました。

### 検査前注意事項の自動表示
メニュー選択時に検査種別ごとの注意事項（例：腹部CT「4時間絶食」、MRI「金属類除去」）が自動表示されます。スタッフが個別に確認する手間を省けます。

### 放射線被ばく線量の簡易メモ機能
mSv・mGy・DLP（mGy・cm）など放射線検査で使用する単位に対応した線量記録機能を実装しました。一般エンジニアには思いつかない医療現場特有の機能です。

### ステータス管理
予約を「受付中→確定→完了」と段階的に管理できます。キャンセル履歴もSoftDeletesにより保持されます。
...
reservations   ：予約情報（上記全テーブルに紐付く・線量メモ含む）
```


## 工夫した点
- 放射線技師としての現場経験をもとに、実務で必要な項目を設計に反映しました
- React + TypeScript + Laravel APIの構成でリアクティブなUIを実現しました
- SoftDeletesを全テーブルに導入し、削除データの復元・履歴管理を可能にしました
- N+1問題を防ぐため `with()` によるEagerローディングを実装しました
- FullCalendar.jsでステータス別に色分けされたカレンダー表示を実装しました
